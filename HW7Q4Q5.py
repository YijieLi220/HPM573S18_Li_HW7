...
#minimu, maximum and the number of samples
...
class Cohort:
    ...
    def simulate(self, n_time_steps):
        ...
        for patient in self._patients:
            ...
            if not (value is None):
                ...
                if value>5:
                    self._fiveyrs.append(1)
                else:
                    self._fiveyrs.append(0)
    ...
    def get_percentage(self):
        return float(sum(self._fiveyrs))/float(len(self._fiveyrs))
...
class Calibration:
    ...
    def get_mortality_estimate_interval(self, alpha, deci):
        """
...
m=Calibration()
m.sample_posterior()
print('The estimate of mortality probability ({:.{prec}%} credible interval) is '.format(1-0.05, prec=0),
      m.get_mortality_estimate_interval(0.05, 4))


Calimodel=MultiCohort(ids=m.cohortids,
            mortality_probs=m.mortalityresamples,
            pop_sizes=[POP_SIZE]*POST_N)
Calimodel.simulate(N_TIME_STEPS)

#Q5
x1=Calimodel._meanSurvivalTimes
sum_statx1 = StatSupport.SummaryStat('X samples', x1)
estimatex1=sum_statx1.get_mean()
projection_interval=sum_statx1.get_PI(0.05)
print('The estimate of mean survival times ({:.{prec}%} projection interval) is '.format(1-0.05, prec=0),FormatSupport.format_estimate_interval(estimatex1,projection_interval,4))
